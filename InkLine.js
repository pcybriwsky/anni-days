        ...
        if (willSplatter && n > splatterDistance) {
          ...
          if(this.shade && n%12 ==0){
            ...
            stroke(c)
              ...
              // line(plotX, plotY - shade, plotX - shade, plotY)
//               let scribbleLine = new InkLine([[0,0,0], [0,0,0]], null)
//               scribbleLine.shade = 0;
//               scribbleLine.setSplatter(0.98 , 0.4 , 2);
//               scribbleLine.setEndBubble(0.4);
//               scribbleLine.setAnalogueness(0.2, 8);
//               scribbleLine.setStops(0);
//             // ink.setDrawFunction(shapeFunctions[i%shapeFunctions.length]);
//               scribbleLine.setDrawFunction({
//   title: "line",
//   formula: (x1,y1, x2, y2, n, points) => {
//       let plotX = (x2 * n) / points + (x1 * (points - n)) / points;
//       let plotY = (y2 * n) / points + (y1 * (points - n)) / points + noise(plotX);
//       return {x: plotX, y: plotY}
//   } 
// })
//               scribbleLine.drawLine(plotX + shade, plotY - shade, plotX, plotY)
              line(plotX + shade, plotY - shade, plotX, plotY)
              line(plotX - shade, plotY - shade, plotX, plotY)
            // line(plotX, plotY, plotX, plotY)
            }
        }
