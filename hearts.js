...
let palettes = [[[47, 72, 88], [214, 40, 57]], [[55, 63, 81], [0, 141, 213]], [[229, 99, 153], [50, 14, 59]], [[252, 186, 4], [165, 1, 4]], [[165, 1, 4], [100, 141, 229]], [[80, 114, 85], [72, 139, 73]]];
...
let lineFunction = {
  ...
  formula: (x1,y1, x2, y2, points) => {
    ...
    for (let n = 0; n < points; n++){
      ...
      let plotY = (y2 * n) / points + (y1 * (points - n)) / points + noise(plotX);
...
let heartFunction = {
  ...
  formula: (x1,y1, x2, y2, points) => {
    ...
    for (let n = 0; n < points; n++){
      ...
      let plotY = centerY - ((0.8*radius)*cos((n/points * 360) - offset) -  (0.35*radius)*cos(2*((n/points * 360) - offset)) - (0.2*radius)*cos(3*((n/points * 360) - offset)) - (0.05*radius)*cos(4*((n/points * 360) - offset)))
      returnArray.push({x: plotX, y: plotY});
    }
    return returnArray;
  } 
...
let shapeFunctions = [lineFunction, circleFunction, spiralFunction, waveFunction, heartFunction]
function preload(){
  ...
  for(let k = 0; k <= rows; k++){
    for(let h = 0; h<= columns; h++){
      ...
      if(offset[i] == undefined){
        offset.push(map(k+h, 0, rows+columns, 10, 100));
      }
      ...
      angleMode(DEGREES)
      currentInkLine.animateLine(x, y , x + offset[i], y, (frameCount-1)*fMulti, frameCount*fMulti); 
      i++;
    }
  }
...
function waterColour(poly, c1, c2, layer) {
  ...
  for(let i = 0; i < numLayers; i ++) {
    if(i == int((layer+numLayers)/3) || i == int(2 * (layer+numLayers)/3)) {
      poly = poly.grow().grow();
